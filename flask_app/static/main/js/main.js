...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Google Maps Scraper button functionality (Desktop)
    const googleMapsScraperBtn = document.getElementById('googleMapsScraperBtn');
    if (googleMapsScraperBtn) {
        googleMapsScraperBtn.addEventListener('click', async function() {
            const userInput = document.getElementById('googleMapsScraperInput').value.trim();
            
            if (!userInput) {
                alert('Please enter what you want to scrape');
                return;
            }

            // Replace all spaces with + for URL-friendly input
            const urlFriendlyInput = userInput.replace(/\s+/g, '+');

            try {
                googleMapsScraperBtn.disabled = true;
                googleMapsScraperBtn.textContent = 'Scraping...';
                ...
                if (response.ok) {
                    ...
                    if (terminalOutput) {
                        ...
                        successLine.textContent = `Google Maps Scraper initiated for: "${userInput}"`;
                    ...
                    const terminalOutput = document.getElementById('terminalOutput');
                    if (terminalOutput) {
                        terminalOutput.innerHTML += `<div class="terminal-line" style="color: #ff0000;">Error: Failed to initiate Google Maps Scraper</div>`;
                        terminalOutput.innerHTML += `<div class="terminal-line">
                            <span class="terminal-prompt">DeployAssistant@3rdspace:~$</span>
                            <span style="color: #ff0000;">FAILED</span>
                        </div>`;
                ...
                console.error('Error initiating Google Maps Scraper:', error);
                const terminalOutput = document.getElementById('terminalOutput');
                if (terminalOutput) {
                    terminalOutput.innerHTML += `<div class="terminal-line" style="color: #ff0000;">Error: ${error.message}</div>`;
                    terminalOutput.innerHTML += `<div class="terminal-line">
                ...
                googleMapsScraperBtn.disabled = false;
                googleMapsScraperBtn.textContent = 'Begin Scraper';
            }
        });
    }

    // Google Maps Scraper button functionality (Mobile)
    const googleMapsScraperBtnMobile = document.getElementById('googleMapsScraperBtnMobile');
    if (googleMapsScraperBtnMobile) {
        googleMapsScraperBtnMobile.addEventListener('click', async function() {
            const userInput = document.getElementById('googleMapsScraperInputMobile').value.trim();
            ...
            try {
                googleMapsScraperBtnMobile.disabled = true;
                googleMapsScraperBtnMobile.textContent = 'Scraping...';
                ...
                googleMapsScraperBtnMobile.disabled = false;
                googleMapsScraperBtnMobile.textContent = 'Begin Scraper';
            }
        });
    }
